...
from .base import PydanticListModel, PydanticModel
...
def pydantic_model_creator(
    ...
    *,
    ...
    :param exclude_pk: Is the primary key included in the schema?
    ...
    if exclude_pk:
        model_description.pop('pk_field')
    ...
    for fname, fdesc in field_map.items():
        ...
        # Any other tortoise fields
            ...
            if field_type is fields.data.CharEnumFieldInstance:
                try:
                    import enums
                    try:
                        value = getattr(enums, str(fdesc["name"]).capitalize())
                        # ptype = value
                        print(ptype)
                        if issubclass(ptype, EnumMeta):
                            print(ptype)
                    except AttributeError:
                        print("No enum matching")
                except ImportError:
                    print("couldn't import enums")
            else:
        ...
        if fname in pannotations:
            ...
            fconfig["python_type"] = ptype.__name__
            if value:
                fconfig["enum_name"] = value.__name__
